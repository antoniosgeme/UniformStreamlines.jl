...
Built-in color symbols: `:norm` (or `:speed`), `:vx` (or `:u`), `:vy` (or `:v`), `:vz` (or `:w`), `:x`, `:y`, `:z`, or pass any `(pos, vel) -> scalar` function.
...
str3 = evenstream(xs, ys, zs, (x,y,z) -> -y, (x,y,z) -> x, (x,y,z) -> 0.3z)
```
